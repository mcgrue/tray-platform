...
function create_audio(label:string, file:string) : HTMLAudioElement
{
  const button = document.createElement('button');
  button.onclick = ()=>{playSound(label)};
...
try {
  ...
  const audio3 = create_audio("audio 3", "./resources/you-did-something-yay.wav")
